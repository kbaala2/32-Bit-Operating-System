...
int32_t execute_terminal(const char* cmd, int terminal_id){
    ...
    tss.esp0 = START_OF_USER - ((prog_counter) * KERNEL_STACK_SIZE) - sizeof(int32_t);    //sets the TSS esp0 to the kernel stack pointer
...
//         set_act_terminal(0); //will need to implement in lib and set up paging
